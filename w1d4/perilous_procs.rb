...
def filter_out(arr, &proc)
  new_arr = []
  arr.each { |n| new_arr << n if !proc.call(n) }
  new_arr
end
...
def every?(arr, &prc) #['squash', 'pea', 'kale', 'potato']
  arr.each do |el|
...
def multi_map(arr,n=1, &proc)
  n.times { arr.map! { |ele| proc.call(ele) } }
  arr
end
...
require "byebug"
def proc_suffix(sent, procs)
  words = sent.split
  words.map! do |word|
    n_word = word
    procs.each { |proc,v| n_word += v if proc.call(word) }
    n_word
  end
  words * ' '
end
...
def proctition_platinum(arr, *prcs)
  ...
  prcs.each_with_index do |prc, i|
    return_arr = proctition_2(arr, prc)
    hash[i+1] = return_arr
    arr -= return_arr
  end
  hash
...
def procipher(sent,*arr)
  ...
  new_words = words.map do |word|
    arr.each do |procs|
      procs.each do |proc1_k, proc2_v| 
        #debugger
        word = proc2_v.call(word) if proc1_k.call(word)
      end
    end
     word
  end
